  ...
  return (
    <div className='flex items-center justify-center px-3 sm:px-4 py-10 sm:py-14'>
      ...
      <div className={`relative w-[280px] h-[450px] sm:w-[330px] sm:h-[500px]
        md:w-[380px] md:h-[550px] rounded-[32px] sm:rounded-[42px]
        overflow-hidden transition-all duration-500 ${current.bg} ${current.cardBorder}
        ...
        <div className='absolute top-4 right-4 sm:top-5 sm:right-5 z-30 flex items-center gap-2'>
          <button onClick={() => setTheme("dark")} className={`w-4 h-4 sm:h-5 sm:w-5
            rounded-full bg-[#050816] border transition-all cursor-pointer
            ${theme === "dark" ? "border-purple-400 scale-110" : "border-white/20"}`} />

          <button onClick={() => setTheme("light")} className={`w-4 h-4 sm:h-5 sm:w-5
            rounded-full bg-white border transition-all cursor-pointer
            ${theme === "light" ? "border-purple-400 scale-110" : "border-gray-300"}`} />

          <button onClick={() => setTheme("glass")} className={`w-4 h-4 sm:w-5 sm:h-5
            rounded-full bg-gradient-to-br from-white/80 to-white/20 border transition-all cursor-pointer
            ${theme === "glass" ? "border-white scale-110" : "border-white/20"}`} />

          <button onClick={() => setTheme("neon")} className={`w-4 h-4 sm:w-5 sm:h-5
            rounded-full bg-gradient-to-r from-emerald-400 to-green-500 border transition-all cursor-pointer
        ...
        <div className='relative z-20 flex flex-col items-center justify-between h-full px-5 py-6 sm:px-7 sm:py-8'>
          ...
          <div className='relative mt-1'>
            ...
            <div className={`relative w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32
              rounded-full bg-gradient-to-br ${current.orb}
            ...
            <div className='text-center mt-6'>
              ...
              <p className={`mt-4 text-[13px] sm:text-[15px] md:text-[16px]
                leading-6 sm:leading-7 max-w-[280px] mx-auto ${current.sub}`}>
          ...
          <div className='relative mb-1'>
            ...
            <button className={`relative w-14 h-14 sm:w-16 sm:h-16 md:w-[72px] md:h-[72px]
              rounded-full bg-gradient-to-br ${current.button} ${current.micGlow}
